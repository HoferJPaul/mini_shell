...
/*   Updated: 2026/03/05 17:01:26 by phofer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
** Reads pipe content into a newly allocated string.
** Used by the parent to retrieve the heredoc content the child collected.
*/
...
/*
** Child process: reads lines until delimiter or EOF, writes content to pipe.
** SIGINT calls _exit(130) directly — guaranteed death even if readline
** internally overrides SIG_DFL before we get to readline().
*/
...
/*
** Forks a child to collect heredoc input.
** Parent waits: if child was killed by SIGINT, sets g_sigint_received and
** returns NULL. Otherwise reads content back from the pipe.
** termios_change suppresses terminal echo of ^C during input.
*/
static char	*heredoc_readline(const char *delim, int no_exp, t_shell *mini)
{
	...
	termios_change(1);
	/* always reset readline's idea of the terminal after child used it */
	rl_on_new_line();
	rl_replace_line("", 0);
